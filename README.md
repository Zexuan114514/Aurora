...
- **设置**：右上角齿轮打开的是**整页设置**（外观 / 资料源 / 简介翻译 / 网络 / 转区启动 / 关于 六个页签），毛玻璃强度、背景暗化、强调色、玻璃饱和度、Ken Burns、简介语言都在「外观」里，改动实时生效。详见 [设置界面](#设置界面)。
...
改动立刻生效并写进 `data\library.json`；按 `Esc` 或点左上角「← 返回」回到大厅，焦点仍停在原来的游戏上。
...
| `e2e.py` | **46/46 通过**（含大厅横滑、焦点居中、单击进入、Esc 返回、双击启动、缩放滑杆、整页设置六页签、转区面板与兜底启动、最小窗口下设置页不溢出） |
