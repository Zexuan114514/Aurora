...
| WillPlus/AdvHD（如 少女之剑） | **可用（专用 hook 码，已固化）** | Textractor 自带的 WillPlus 系钩子对不上这类 exe：`WillPlus` 报 `function call not found`、`WillPlusW/A` 找不到特征码、`WillPlus2` 把地址算到 **Intel 显卡驱动的 `igc32.dll`** 上（吐乱码），只剩按字形抓的 `GetGlyphOutlineW`，而引擎字形有缓存 → 一行只抓到零星几个字（实测「…打ち明けた」只抓到「出会　打明」）。**出路是给引擎文本函数直接指定地址**：`HQ-4@A22E:AdvHD_crack.exe`（`Q`=UTF-16 字符串、`S`=字节串、`V`=UTF-8；地址来自 LunaTranslator 日志的 `注入钩子: WillPlus3 0040A22E`，减去基址 0x400000）。Aurora 已把这条实测记录写进指纹表（文件名+字节数+CRC32 匹配才启用），开翻译时自动带上；实测同一句的 GDI 缺字版会被并掉（merged=9、审计漏掉 0）。拿不到地址时仍可退回 OCR 模式 |
