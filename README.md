...
- **左栏目录**：全部游戏（N）→ 未分类（N）→ 自定义分类（＋ 新建，选中的分类可以上移 / 下移 / 重命名 / 删除）→ 按状态（在玩 / 通关 / 搁置 / 未标记）→ 按开发商（按数量排序，超过 12 个可展开）。底部写明「一部游戏可以放进多个分类；删除分类只解绑，不动游戏与记录」。
...
- **作用域会带回主页**：在左栏选「柚子社」这类范围后回主页，大厅只滑这一类，左上角出现胶囊「柚子社 · 3 ×」，点它回到全部。作用域会被记住（`localStorage`）。
...
| `e2e.py` | **67/67 通过**（含大厅横滑、按坐标真点封面、拖动后单击仍能进入、设置页输入不穿透、分类工作区新建/命名/排序/删除、整理模式多选归类、主页作用域过滤与胶囊清除、状态徽标、浅色主题与配色切换） |
