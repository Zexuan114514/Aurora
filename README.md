...
- **多条钩子线程同时吐同一句**：同一个引擎常有 2~4 条线程在吐（RIDDLE JOKER 是两条同名 `KiriKiriZ`：一条逐字×3、一条×2）。处理顺序很关键：**先过线程门禁、再去重登记** —— 顺序反了，被门禁丢掉的那句会先污染「最近去重表」，真身线程随后送来的同一句就被当成重复吞掉，整句消失（表现是「一句有译文、一句根本没出现」，RIDDLE JOKER 实测复现并已修）。门禁也只丢**不像台词**的输出（菜单动画、打字中途的半截碎片），像台词的一律走去重合并。
