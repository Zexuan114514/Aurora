...
主界面是 **绕竖轴转的一圈大封面**（循环队列）：焦点封面最大、正对用户，两侧依次缩小并向轴内倾斜，背景同步淡入该游戏的壁纸；单击进入游戏页，主页启动按钮或右键菜单可直接开玩。
...
![Aurora 大厅：封面绕竖轴排开，焦点封面最大](docs/images/hall.png)

- 所有封面排在一根竖轴的圆周上：焦点那张最大、正对用户并带强调色描边，
  两侧依次缩小、向轴内倾斜（`rotateY` + 透视），越远越暗、边缘还会轻微虚化，看上去像整圈封面在眼前转动。
- **首尾相接的循环队列**：从最后一张继续往右（或从第一张往左）会绕回另一头，没有尽头；末尾的「＋ 导入游戏」方块也在环上。
- **两种主页布局可选**：设置 → 外观 → **主页布局**，可以在「环形队列」（默认）和「平铺横滑」（最初的 NS 大厅：一横排、焦点封面放大上浮、走到头即止）之间切换，改完立刻生效并记进 `data\library.json`。

![Aurora 平铺横滑大厅（设置里可切换的老布局）](docs/images/hall-flat.jpg)
- `←` `→` / 滚轮 / 横向拖动都能切换，`Home` / `End` 直接跳到首尾，`Enter` 进入游戏页，主页启动按钮或右键菜单直接启动，`Ctrl+F` 搜索；
  鼠标悬停 0.3 秒也会自动聚焦。拖动时封面跟着指针转（抓住封面本身也能拖），松手自动吸附到最近的一张。
- 左上角是 **获取游戏** 入口；环上的虚线方块用来「导入本地游戏」或「获取游戏」；
...
- 都匹配不上时不会硬猜：弹出候选列表（标注来源、按匹配度从高到低排序，**点哪条才应用哪条**），也可以自己输入关键词重搜。
  已经匹配好的游戏也能随时从「⋯ → **手动匹配…**」打开这个面板换一条：面板会预填当前名字，
  旁边给出文件名推断、中文名、日文原名等快捷词；按「搜索」只会把候选列出来，不会自动替换当前匹配。
...
**图片加载的回退链**（`gl/sources/steam.py` + 前端 `frontend/src/views/HallView.vue`）
...
- **环形大厅**：主界面是一圈绕竖轴排开的封面（循环队列），焦点那张最大、正对用户并高亮，
  两侧依次缩小、倾斜、压暗；背景同步换成焦点游戏的壁纸。
  `←` `→`、滚轮、横向拖动都能切换，首尾相接绕圈，环上还有一个「＋ 导入游戏」色块。
- **游戏页**：单击封面进入，壁纸铺满整屏，左下角是 LOGO / 标题 / 简介与按钮；左上角「← 返回」或 `Esc` 回大厅，
  在游戏页里按 `←` `→` 仍然可以直接换游戏。
- **打开方式**：单击 = 进入游戏页看简介；主页启动按钮、游戏页「开始游戏」或右键菜单负责启动；空库时显示导入引导卡片。
- **五套主题**：极光玻璃、展签式画廊、夜间放映厅、收藏架、Atelier 工作台都可运行时切换，并分别提供深色 / 浅色态。模糊强度是每套主题的令牌：极光保留毛玻璃，其余主题用细线、纸纹、黄铜或纸片材质建立身份。
- **背景缩放**：背景面板底部有缩放滑杆（100%–300%）和「复位」按钮，每款游戏的缩放分别记住。
- **无边框窗口**：调用 DWM 的圆角 / 暗色边框 / 投影，并接管拖拽与八向缩放（`gl/winapi.py` + `frontend/src/core/window.ts`）。窗口拖动请用顶部工具条或大厅底部信息带。
- **设置**：右上角齿轮打开的是**整页设置**（外观 / 资料源 / 简介翻译 / 网络 / 转区启动 / 游戏内翻译 / 插件 / 关于 八个页签），主题模式、**主页布局（环形队列 / 平铺横滑）**、配色预设、毛玻璃强度、背景暗化、强调色、玻璃饱和度、Ken Burns 都在「外观」里，改动实时生效。详见 [设置界面](#设置界面)。
...
- **更多菜单**：加入/取消收藏、设置自定义图标、打开所在文件夹、设置启动参数、**转区启动…**、重新搜索游戏信息（够准就直接采纳）、**手动匹配…**（按名字搜，候选按匹配度列出、点一条才应用）、翻译简介、打开商店页面、从库中移除（**只移除记录，不删文件**，同时清掉该游戏留下的自定义图标与本地背景图副本）。
...
| 游戏内翻译 | Textractor / OCR、自动启动、上下文行数、术语表、悬浮窗与翻译状态 |
| 插件 | 插件状态、权限、来源、失败信息、重新扫描 |
...
- **五套视觉主题**：极光玻璃、展签式画廊、夜间放映厅、收藏架、Atelier 工作台；每套主题都有深色 / 浅色态，按钮语言由对应的独立皮肤维护。
...
- **悬浮窗**：置顶小窗，默认**鼠标穿透**不挡游戏；`Ctrl+Alt+T` 切换穿透/可拖拽，`Ctrl+Alt+Y` 显示隐藏；支持仅译文/双语、字号与不透明度调节。每次开翻译都回到默认的 760×150 大小（不沿用上次的坐标，避免高 DPI 下越飘越偏），**拖动标题栏移动、拖右下角手柄或任意边缘缩放**（无边框窗口默认没有可拖的边框，Aurora 会补上 `WS_THICKFRAME`）。游戏退出时自动停止并收起。
- **入口**：游戏页「翻译」按钮打开状态面板（开关、引擎、PID、线程、hook 码、重译、框选 OCR 区域、最近译文）；设置 → **游戏内翻译** 页签管引擎、TextractorCLI 路径、OCR 组件状态、上下文句数、术语表与悬浮窗样式。
- **务必配合转区**：日文原版在中文系统下（Shift-JIS 被按错误代码页解释）产出的文本本身就是乱码，钩子拿到的自然也是乱码——表现为「翻页时夹杂乱码、译文和当前文本对不上」。**先在「⋯ → 转区启动…」里开启转区再开翻译**，文本才正确；Aurora 也会在没开转区时于翻译面板里提醒你，并在翻译失败时保留上一句译文而不是把屏幕刷成乱码。
- **引擎矩阵与排错**（我们自己的实测记录，不用 Textractor/LunaHook 的引擎表）：
  完整清单（已通过测试 / 待测 / 没有钩子的 OCR 靶子）见 **[docs/engines.md](docs/engines.md)**；下面只列已经真机跑过的：
| 引擎 | 状态 | 建议 |
| --- | --- | --- |
...
| BGI/Ethornell（如 秽翼的尤斯蒂娅） | **可用（已固化规则）** | 同一进程里有两条线程一起吐同一句：引擎级 `BGI` 钩子给完整正文（`視界を黒い何かが塞いだ。`），`TextOutA`/`ExtTextOutW` 这两条按字形抓的 GDI 钩子给**缺字版**（`視界黒塞`）和乱码版 —— 以前同一句会被翻两遍（先错的再对的）。现在 Aurora 会把这一批候选压住 0.6s，等两个版本都到齐后只翻完整那份。日志里出现 `BGI2: pattern found but the function offset is invalid` 是正常的：Textractor 的 BGI2 专用钩子认不出这个版本，真正出文本的是随后注入的通用 `BGI` 钩子，**不需要手动选钩子** |
| WillPlus/AdvHD（如 少女之剑） | **可用（两级方案）** | Textractor 自带的 WillPlus 系钩子对不上这类 exe：`WillPlus` 报 `function call not found`、`WillPlusW/A` 找不到特征码、`WillPlus2` 把地址算到 **Intel 显卡驱动的 `igc32.dll`** 上（吐乱码），只剩按字形抓的 `GetGlyphOutlineW`，而引擎字形有缓存 → 一行只抓到零星几个字（实测「…打ち明けた」只抓到「出会　打明」）。现在有两条路：① **实测 hook 码**：`HQ-4@A22E:AdvHD_crack.exe`（`Q`=UTF-16、`S`=字节串、`V`=UTF-8），Aurora 按「文件名+字节数+CRC32」自动带出，实测 5 句连续翻页全完整；② **内存补全**（换别的 AdvHD 作品也不用再找地址）：缺字版的字按顺序都在真句里，而真句就在进程内存 —— 只读扫内存按子序列匹配取最紧凑那句，`つの物思耽で 内で我返`→`いつのまにか物思いに耽っていたようで、機内アナウンスで我に返る。`、注音混入的 `空木うつぎ日向ひなた１年間留学え本へ飛行中`→`オレ、空木日向は、…` 都能还原；4 字级极短残片偶尔配不出，就保留原样。仍拿不准时还能退回 OCR |
...
- **多条钩子线程同时吐同一句**：同一个引擎常有 2~4 条线程在吐（RIDDLE JOKER 是两条同名 `KiriKiriZ`：一条逐字×3、一条×2）。处理顺序很关键：**先过线程门禁、再去重登记** —— 顺序反了，被门禁丢掉的那句会先污染「最近去重表」，真身线程随后送来的同一句就被当成重复吞掉，整句消失（表现是「一句有译文、一句根本没出现」，RIDDLE JOKER 实测复现并已修）。门禁也只丢**不像台词**的输出（菜单动画、打字中途的半截碎片），像台词的一律走去重合并。**冷启动 30 秒内门禁放宽**：游戏刚起来、领跑线程还没选出来时连点翻页，前几句往往先从别的线程到，以前会被门禁整句丢掉（真机自测「原始行审计漏掉 N 条」就是这么来的）；这 30 秒里只挡不像台词的行，台词照收、去重照做（日志 `vntext gate held (cold start)`）。
- **同一句的「完整版 + 缺字版」只翻一次**：有的引擎（实测 **秽翼のユースティア / BGI**）里，引擎自己的钩子吐完整句、按字形抓的 GDI 钩子（`TextOutA`/`ExtTextOutW`）吐缺字版，两条线程几乎同时到达 —— 以前同一句会被翻两遍（先缺字的错误译文、再完整句的正确译文）。现在候选行会先压住 0.6s（`variant_settle`），等这一批的各个版本到齐后，按「缺字版是完整版的**子序列**」把缺字的那份并掉，只翻完整那份；反过来（完整版先到）也一样。判定还额外排除了「短句正好是长句的开头」这种正常接续（`誰か。` 不会被 `誰か説明してほしい` 吞掉）。
- **噪声行不再连坐整条线程**：以前一条 `─` 分隔线、一个菜单词就会把该线程标记成「系统刷屏」、后续所有文本全丢（实测 BGI 的缺字版线程就这样被误杀，偶尔还会误伤真正的文本线程）。现在只有「同片段重复 ≥4 次 / 超长无句读资源表」这类**真刷屏**才整线程丢弃；普通噪声行只丢自己，只有「噪声攒够 3 条且从来没吐过台词」才当系统线程处理。
- **专用 hook 码（WillPlus/AdvHD 这类引擎）**：Textractor 自带的引擎钩子在「改过的 exe」上常常挂错模块，这时可以在游戏页「翻译」面板里直接填一条用户钩子码，格式是 `H<模式><偏移>@<模块内偏移>:<exe文件名>`（`Q`=UTF-16 字符串、`S`=字节串、`V`=UTF-8；语法取自 Textractor 的 `host/hookcode.cpp`），点「存为专用」后会写进这个游戏的记录、以后开翻译自动带上。**实测例子**：少女之剑（AdvHD_crack.exe）用 `HQ-4@A22E:AdvHD_crack.exe` 就能拿到完整正文。本机验证过的作品 Aurora 会按「文件名+字节数+CRC32」自动带出这条码；**发码要等 attach 生效**（CLI 打印「管道已连接」之后再发），跟着 attach 同一批写进去会把 CLI 顶掉、一行文本都收不到 —— 这个坑已经在代码里处理。
- **缺字版自动补全（不用再去找地址）**：同一类引擎里，按字形抓的 GDI 钩子只会吐出「新字形」，一句话会缺字；但**缺的字都是之前见过的字**，也就是说缺字版的每个字都在真句里按顺序出现 —— 而真句此刻就在游戏进程内存里。所以 Aurora 会（只读）扫一遍内存，按「子序列 / 公共子序列 / 只比汉字」三档匹配找出最紧凑的那句，把缺字版补成完整台词：实测 `つの物思耽で 内で我返` → `いつのまにか物思いに耽っていたようで、機内アナウンスで我に返る。`，注音混进去的 `空木うつぎ日向ひなた１年間留学え本へ飛行中` 也能还原。**全程只读**（OpenProcess + ReadProcessMemory），不注入、不改内存、崩不了游戏，也不需要 LunaTranslator 那种「先拿到引擎函数地址」的前置条件；命中过的内存区域会记成热区，之后每句只扫热区。全量扫描实测 44~68 秒，所以取词链路最多等 2.5 秒就**先放行缺字版**，扫出来的完整版随后作为**同一句的新版本补发**（那句还没翻页过去就直接换上去；已经翻页了则只把面板里的历史修正过来、不动悬浮窗）；4 字级的极短残片偶尔配不出来，那时保留原样（可手动填 hook 码或改用 OCR）。
- **翻译排队**：新台词不再掐断上一句的翻译请求（掐断就永远没有译文），改成小队列逐句翻 —— 快速连翻时也句句有译文，悬浮窗按原文对号入座、不会倒着跳。
- **折行与说话人名字**：有的引擎（实测 Escu:de 的**悠刻のファムファタル**）把一句话折成好几行发给 Textractor，CLI 会原样逐行打印、**只有第一行带 `[handle:…]` 头** —— 以前这些续行会被当成「另一个匿名线程」的独立台词，于是一句话被拆成好几条依次送翻译、悬浮窗逐条顶掉，**最后只看得见最后一行**。现在续行会并回同一句；把说话人名字单独发一条的引擎（如 `アマリリス`）也不再单独翻，而是并进下一句当 `【名字】`（菜单行如 `ロード` 不会被误当名字）。
- **同进程里的杂讯线程**：白色相簿2 一个进程里同时有乱码线程（没转区时读错编码，吐出 `इव孙ؔ䝬׋孙ؔ灐灒灟灹灱炄灰` 这种天城文/希伯来文混汉字）、菜单栏线程（`画面設定環境設定システムヘルプ`）、视频窗口标题（`ActiveMovie Window`）和视频文件名（`mv01`）。这些以前会被当成台词送翻译、把「当前线程」带偏；现在：混入意料之外文字系统的串判乱码、纯拉丁字母数字的短串（窗口/文件名）判杂讯、进程内所有窗口标题（含子窗口）也当噪声，短台词 `「あ…」` 则照常算台词。
...
- **每引擎的验收模板**：转区启动 → 开翻译 → 连续翻 20 句 → 记录「引擎名 / 线程列表（名字+样例）/ 是否缺字或重复」→ 反馈给我，我据此补规则并归档到引擎矩阵。
...
`Aurora.exe` 的图标来自 `gl/assets/aurora-icon.png`：已去掉原图外围黑底并压缩为 512×512 透明 PNG。`tools/make_icon.py` 将它转换为包含 7 种尺寸（256→16px）的 `aurora.ico`。

想换成自己的图标：替换 `gl/assets/aurora-icon.png`（512×512 透明 PNG）后运行 `python tools\build_exe.py`。打包脚本每次都会重新生成 `.ico`。
打包完成后脚本会通知 Windows 资源管理器清除旧图标缓存并刷新 `Aurora.exe` 所在目录，无需重启资源管理器。
...
│  ├─ assets/aurora-icon.png   程序图标源文件（512×512）
│  ├─ assets/aurora.ico   程序图标（7 种尺寸）
│  └─ web/v2/             Vite 产物：index.html · overlay.html · bundle/ · build-info.json（源码在 frontend/）
...
> `domain`（纯规则：文本清洗/匹配打分/引擎规则/会话规则）、`app`（端口 + 事件 + 七个用例服务）、
> `infra`（JSON 状态存储、任务执行器、TextractorCLI、进程会话、下载监听、配置、翻译队列）、
> `platform`（Win32 原语：窗口/托盘/热键/截图/OCR/进程树/内存）、`ui`（桥接 mixin、悬浮窗、前端静态资源）。
> `gl/` 现在是**兼容层**：已迁模块保留透明模块别名（`gl.x is aurora.*.x`），老调用点与 `tools/` 探针零改动。
...
python tools\e2e.py               # 端到端：导入 → 自动搜索 → 换背景 → 缩放大厅环形队列 → 设置页 → 转区兜底 → 启动/结束 → 拖拽缩放
...
python tools\contrast.py          # 真机对比度自检：5 套风格 × 深/浅 × 设置页/游戏页 = 40 个采样点（正文 ≥4.5、大字 ≥3.0）
python tools\meta_offline.py      # 用本地缓存校验元数据解析（断网时也能跑）
python tools\check_bridge.py      # 前端调用的方法与后端实现是否一一对应（扫 frontend/src 主窗 + 悬浮窗）
python tools\check_library.py     # 检查现有游戏库是否完好（id / name / exe、重复 id；在 state\ 下读）
python tools\checks\run_all.py    # 离线检查全家桶（14 项）：契约快照 / 依赖白名单 / 去敏夹具 / 探针清单 /
                                  #   基线漂移 / 桥接转发目标 / 引擎规则包 / 分层守卫 / 打包清单 / 启动冒烟 /
                                  #   前端构建指纹 / 主题契约（含令牌值域）/ 前端测试面 / 设置键名
...
python -m pytest                  # 同一批离线检查 + domain 金样本 + 插件 / 诊断包回归（127 个用例）
...
python tools\theme_probe.py       # 主题自检：4 套配色预设 × 3 档模式，面板底色与文字对比度实测
...
| `e2e.py` | **101/101 通过（0 skipped）**（含环形队列首尾相接、封面越远越小且带倾斜、焦点封面居中、手动匹配只列候选不自动采纳、点候选才应用、按坐标真点封面、拖动后单击仍能进入、设置页输入不穿透、分类工作区新建/命名/排序/删除、整理模式多选归类、主页作用域过滤与胶囊清除、状态徽标、浅色主题与配色切换、主页布局切换、专用 hook 码的存取与校验、**插件区状态与权限、「重新扫描插件」回执、简介翻译走 `plugin:<id>`、诊断包导出落盘、末尾方块直接进本地导入、窗口按钮贴右缘 + 两个大厅入口在顶部栏内、主页启动按钮与右键菜单**） |
...
| `vntext_probe.py` | **全部通过**（假 TextractorCLI 走真实 UTF-16 协议：attach、逐行解析、菜单噪声过滤、线程自动选择与锁定、手写 hook 码透传；本地假 LLM 验证 SSE 流式、上下文、术语表注入、缓存命中与暂停；**DRACU RIOT 真机四行原文的三形态清洗**；**秽翼のユースティア 真机样本的「完整版 + 缺字版」并合**（含反向顺序、短句不被长句吞掉、噪声行不误杀线程）；**WillPlus 专用 hook 码**（指纹匹配、H-code 拼装与校验、开翻译自动发码、含空格残片与「残片被当名字」两种漏网形态）；OCR 缺日语组件时给出明确错误；窗口捕获与相对区域裁剪） |
...
| `vntext_live.py`（RIDDLE JOKER） | **全部通过**（两条同名 KiriKiriZ 线程交替抢先：7 句台词全部干净、7 句译文全部返回；**原始行审计：认出 14 / 只有人名 4 / 噪声 3 / 半截碎片 5 / 漏掉 0**） |
| `vntext_live.py`（悠刻のファムファタル） | **全部通过**（Escu:de 引擎把长句折成 3 行 + 说话人名字单独一条：6 句台词全部干净、6 句译文全部返回，名字已并成 `【アマリリス】「…」`；原始行审计漏掉 0） |
| `vntext_live.py`（秽翼のユースティア） | **全部通过**（BGI/Ethornell：从读档点连翻 7 次 → 6 句台词全部是**完整版**、6 句译文全部返回；原先每句都会先来一份 `TextOutA` 缺字版（`視界黒塞`／`何かちを肉塊に変ている`），现在合并计数 `merged=4`、原始行审计「半截碎片 4 / 漏掉 0」） |
...
| `checks\run_all.py` | **14/14 通过**（契约快照 151 方法 / 14 事件 / **103 个前端调用点**、运行时依赖白名单、去敏夹具 24 游戏 / 63 字段 / 123 会话、探针清单 32 个脚本〔控制台守卫 + 真跑 offline 脚本 + 路径漂移守卫〕、架构基线漂移、桥接转发目标、引擎规则包、分层守卫、打包清单、启动冒烟，以及 v2 新增的前端构建指纹 / 主题契约与截图基线 / 前端测试面 / 设置键名） |
| `pytest` | **127 passed**（离线检查 + domain 金样本 + 数据 v2 迁移/去抖/回滚 + 插件加载与翻译引擎 + 诊断包脱敏 + 系统提示不算台词 + 取词链路回归网〔状态推送去抖 / 悬浮窗非阻塞 / 缺字补全门禁与补发 / 线程门禁冷启动宽限期 / 文学重复〕） |
| `build_exe.py --dry-run` | **通过**（前端 v2 模块树共 69 个文件、图标、内置规则包、winrt 7 个动态声明；CI 每推一次都跑） |
