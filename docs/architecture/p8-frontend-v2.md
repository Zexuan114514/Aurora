# P8 交付记录：前端 v2（Vue 3 + Element Plus + 五套主题）

> 最新续做：**P8.21 找钩子事实口径与线程诊断（2026-09-26）**。五套 Vue 模板与应用共用
> `<theme>.buttons.skin.css`；见本文末尾。P8.16 的撤回记录保留为历史。
...
视觉上是**五套可切换风格 × 深/浅两态**：极光玻璃（默认）/ 展签式画廊 / 夜间放映厅 / 收藏架 / Atelier 工作台。
布局只有一套（`src/styles/layout.css`，由 v1 的 `app.css` 机械迁移），
风格全部是语义令牌的不同取值 —— 加一套风格 = 加主题令牌、签名和按钮皮肤文件，不动组件几何。
...
线程状态公开真实 key、累计条数、最新文本、Hcode 与最近 6 条原文，翻译面板点击线程即可展开诊断。
使用者已实测提示与线程详情通过；针对新增的小窗口溢出问题，线程卡片区设置独立滚动，翻译面板自身也允许纵向滚动，
专用 Hcode、找钩子和历史区在条目较多时仍可到达。
...
不够 → 改成「改造收藏架」。预览图已生成（`preview/atelier.jpg`）。

---

## P8.15 五套示例页的启动键（2026-09-23 夜）

> 编号说明：同一晚另外几批工作（真机门补跑、极光浅色补完、画廊顶部遮罩、主页交互与浅色画廊）
> 记在文末「还没做的」下面，占掉了 P8.12–P8.14；本节按路线图的编号顺延为 P8.15。

反馈「给主题 demo 设计风格化的按钮，必须有一套区分度高的启动键」。
先读了参考项目 [gal-launcher](https://github.com/KamiNeko-pre/gal-launcher) 的
`src/themes/*.css`（只看设计语言，不抄实现）：它的手法是把「启动」当成整套主题的仪式核心 ——
Arcade 的 START 是全屏唯一**可下压 + 呼吸灯**的键，Atelier 的 play-stamp 是全页唯一
**正圆红印章**，用「形状 / 颜色 / 描边 / 光影 / 质感 / 姿态」六重差异把它拎出来。

按同一思路给五套示例页各设计了一个启动键，**共用同一份 DOM**（ADR-0013 的原则）：

| 主题 | 一句话 | 形状 | 关键手法 | 动效 |
| --- | --- | --- | --- | --- |
| 极光玻璃 | 一条会流动的极光光带 | 胶囊 58px | 内部斜向青→紫渐变带 + 青色外发光 | 光带常驻横移 5.2s，hover 提速到 1.9s |
| 展签式画廊 | 盖在展签上的印章 | 直角矩形 | 墨色外框 + 内缩 5px 的第二道细框 | hover **整块反白**（墨底纸字）+ 内框收紧；左侧竖排编号 `01` |
| 夜间放映厅 | 放映机点亮 | 琥珀实色 56px | 灯珠（径向渐变 + 白芯）+ 下缘 3px 时间码刻度 | 常驻呼吸辉光 2.6s；hover 停住呼吸、辉光撑满 |
| 收藏架 | 一册线装书 | 黄铜实色 54px | 左侧 14px「书脊」带缝线 + 压印内阴影 | hover **向右抽出 3px** |
| Atelier | 压进纸里的红印章 | 正圆 138px | 双层白描边 + 光晕 + 油墨颗粒 + 外圈**不规则油墨飞溅** | 常驻歪 7°，hover 回正 + 放大 1.06 |

验收（用 **headless Chromium 真渲染**，不是靠读代码）：
五套静止态 + 五套 hover 态逐张截图核对（`docs/theme-demos/preview/buttons-launch.jpg`
与 `buttons-hover.jpg` 是并排对比图），全页预览图一并重渲（新增 `atelier.jpg`）。
结构守卫 `_sandbox/verify_demos_static.py` 增加了启动键检查
（每页恰好一个 `button.play`、三个部件齐全、CSS 里本体 + 三部件 + `:active` 都在）——
五个页面全过。

**可读性实测（离线）**：又写了 `_sandbox/check_demo_button_contrast.py` —— headless Chromium
渲染 + WCAG 相对亮度，取**文字左右紧邻处的背景中位色**（中位数不受抗锯齿与装饰线影响），
阈值按项目口径（正文 4.5 / 大字 3.0）。**抓到一个真问题**：Atelier 的印章原来是浅玫红
（`#e2899f → #d4738e`），白字压上去只有 **2.45–2.73**，9.5px 的 ENTER 根本读不清；
改成朱砂色阶（`#b8506c → #9c3f5a → #6d2537`）后升到 **4.60–5.34**，
而且更像真印章 —— 印章本来就该比桌面上的玫红深一档。五套最终值：
极光 6.15 / 画廊 16.41 / 放映厅 7.83 / 收藏架 5.34 / Atelier 5.34（最贴线的是 Atelier 图标 4.60）。

---

## 历史收尾记录与当前待办

本节前半保留 P8.11–P8.18 的阶段证据；这些阶段的待跑项目已由 P8.19 收口，当前状态以
上面的快照和文末 P8.19 为准。
...
### P8.15（2026-09-23 深夜）：右键菜单 + 删「悬停即切换」（反馈第 19-b 条）

| 项 | 内容 |
| --- | --- |
| 右键菜单 | `#hallMenu`（`class="menu glass"`，复用既有菜单样式与 `pop` 动效；`position: fixed` 跟点击点、贴边夹回窗口内）。四种布局共用一套：环形 / 平铺挂在 `.gi` 封面、列表挂右栏行。条目 **启动游戏 / 加入收藏（已收藏显示「取消收藏」）/ 详情 / 移除游戏** —— 移除沿用游戏页那套 `modal` 确认 + `remove_game` + `emitUi("library:refresh")`，不删磁盘文件。收起时机：点空白（外壳全局 click）、Esc（`closeAllPanels`，菜单位加进 `state.menus.hall`）、滚轮滑动、切换布局 |
| 删「悬停即切换」 | `ring.ts` 里那段 320ms `mousemove` 定时器整段下掉（它会抢焦点，也挡住右键菜单的使用）；滑动只留滚轮 / ← → / 拖拽 / 环形浮动这些显式动作 |
| 判定 | `e2e` 新增 5 条：右键弹出菜单（四条 `data-act` 齐不齐）、悬停不再切换焦点、菜单里能启动、菜单里能收藏（点完还原原状态）、菜单里能进详情且菜单收起 → **101/101**；前端表面快照 98 → **99 个 id** |

门：主题矩阵 **25/25 偏差 0**（**没有重录基线** —— 菜单默认隐藏，25 张画面一张没动，正好
...
「一套主题可多张皮肤」的契约放宽（守卫与 spec 仍逐张查）。**当时**按钮风格化重新记为待办；
P8.19 已按小步方案完成五套按钮皮肤，教训写在 `docs/theme-templates/README.md` 最后一节。
...
**历史记录**：P8.18 当时把按钮风格化列为下一轮；P8.19 已按小步方案为五套主题接入独立
`<theme>.buttons.skin.css`。悬浮窗整套跟主题仍按使用者决定暂缓。

### 当前仍开放的事项

0. **真机翻译链补验**：少女之剑的缺字补全 / 修订回发，以及冷启动连点翻页的 0 漏句审计。
1. **RIDDLE JOKER 复测**：确认双同名线程与冷启动宽限期在真实游戏上的表现。
2. **书架页与存档管理**：跨分类总览、存档目录快捷方式、手动备份 / 恢复仍未进入当前范围。
3. **低优先级能力**：手柄、多主题扩展、Magpie 集成、CI 自动发布。
4. **悬浮窗跟主题**：当前只修复 v2 按钮可见性，整套主题跟随仍暂缓。
5. 前端源码发生变化后，仍须先构建 `gl/web/v2/`，再按需执行
   `python tools\build_exe.py`；入库的是产物树，exe 本身不进仓库。

## P8.19（2026-09-24）：五主题按钮模板与独立按钮皮肤

按使用者要求为五套主题设计 Vue 模板，并应用到 Aurora。交付入口：
[`theme-templates/README.md`](../theme-templates/README.md)，其中有五个
`*-themed-button.vue`、可直接打开的 `preview-themes.html` 和深浅模式总览图。

| 主题 | 一般操作 | 启动键 |
| --- | --- | --- |
| 极光玻璃 | 玻璃胶囊、薄高光边 | 青紫折射边，悬停轻抬 |
| 展签式画廊 | 直角墨线展签 | 双线内框随悬停收紧，无外投影 |
| 夜间放映厅 | 琥珀底线、窄体排印 | 指示灯、胶片刻度，悬停点亮 |
| 收藏架 | 黄铜边与纸面压线 | 两道书脊线，悬停向右抽出 2px |
| Atelier | 虚线纸签 | 56px 高椭圆印章，悬停回正；确认按钮仍为方形纸片 |

实现以 `frontend/src/styles/themes/<theme>.buttons.skin.css` 为唯一来源：
模板通过 `<style src>` 直接引用；应用在 `app.css` 中统一引入。原来五张
主题签名 skin 中的按钮覆盖已迁出，避免旧的渐变、投影和运行中规则交叠。
五张新皮肤均小于 200 行，选择器按主题隔离，保留现有 id、事件和原生按钮语义。

所有主题补齐 hover / active / focus-visible / disabled / running。
启动时文案一直可见；禁用时无位移；运行中使用中性描边；减少动态效果时关闭
位移与过渡。普通按钮的字体继承主题。浅色主按钮采用强调色的 80% 深化色，
三种自选强调色覆盖改用深墨字，避免白字落在亮青、粉、橙上。

这一轮检查中修了两处实际问题：

1. 首次真窗口截图发现 `.btn { position: relative }` 覆盖了返回按钮的绝对定位，
   导致返回键铺成横条并挡住标题。定位已仅保留在需要伪元素的 `.btn.play` 上；
   再次截图确认返回键回到左上角，详情页操作栏正常。
2. 第一版极光浅色主按钮对比度只有 **4.35**；压暗底色后，五主题 × 深浅 ×
   四种强调色 × 四状态 × 四类可用按钮共 **640 个采样全部通过**，最低 **4.94**。
   状态采样使用 Chromium 计算色（包含 `color-mix`），并以同一皮肤的状态选择器
   模拟悬停、按下、键盘焦点；禁用态不计入正文对比度门槛。

已验证：五个 Vue SFC 编译通过；`typecheck` 通过、`vitest` **40/40**、
`pytest` **127/127**、`run_all` **14/14**、`e2e` **101/101（0 skipped）**。
主题截图已重录并独立复跑：**35/35，最大偏差 0**，主题区分度 **23.9**。
前端源文件 **64 → 69**，产物与 `Aurora.exe` 均已重新构建。
