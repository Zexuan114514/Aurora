...
| **P2 数据 v2** | 让数据可迁移、可恢复、写入成本降到常量级 | `state/{settings,library}.json` + `state/sessions.jsonl`；迁移器 + 备份 + `--check`；单写者 + 去抖 | 迁移全等断言；强杀不损坏；回滚可用；导入/导出兼容 | 用备份回到 v1 文件；代码回退版本 |
| **P3 用例服务化 + 桥接 mixins** | 拆掉 2072 行的 `api.py`，契约零漂移 | `app/services/*`；`ui/bridge/*` mixins；`TaskRunner`、`EventBus` | 契约 diff = 0；`check_bridge.py` 无缺失；真机启动/翻译/钩子查找抽测通过 | 按 mixin 粒度回退到旧方法实现 |
...
| P2–P7 | 待开始 | 每阶段结束按「固定动作」四项检查后，把状态与证据补进本表 |
