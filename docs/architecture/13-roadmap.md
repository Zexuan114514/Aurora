...
| P8.16 按钮风格化（画廊） | ⏸ 已撤，重新记为待办（2026-09-24 凌晨） | 两份 Uiverse 样式（均 MIT）曾落地到 `gallery.buttons.skin.css`，使用者实机反馈「观感不如改动前」→ 应用已下线、`#btnHallPlay` 恢复纯文字、主题基线回滚（复跑 25/25 偏差 0）。**保留**：`docs/theme-templates/` 的模板与三态预览、四个落地口径（保留原貌 / 跟强调色 / 跟主题字体 / 尺寸动效先保留）、以及「一套主题可多张皮肤」的契约放宽。下次试的重点：先看它和画廊「1px 直角 + 无投影」的整体语言怎么共处，别整张皮肤一起上 | 待办 |
