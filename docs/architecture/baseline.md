...
| 指标 | 基线 |
| --- | --- |
| 公开桥接方法 | 105（P0 记录）→ **106**（P2 新增 `shutdown()` 生命周期钩子；前端调用点仍 96） |
| 内部方法 | 41 |
| 悬浮窗桥接方法 / 动作 | 4 / 8 |
| 事件主题 | 14 |
...
| 开发 / 打包工具依赖 | Pillow（主题截图与 PNG → ICO；`make_icon.py` / `build_exe.py` 会检查并安装）；运行时 exe 不依赖它，运行时硬依赖仍只有 pywebview + winrt |
