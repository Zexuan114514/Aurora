...
| **Artemis / Emote（x64，引擎自绘文字）** | アマカノ３（甜蜜女友 3） | ✅ 可用（**用户钩子码**，已固化） | **已解决（2026-09-19 实测）**：用 MisakaHookFinder（它内嵌 Textractor 的"文本搜索"：先在内存里找到会变化的文本，再找出读它的那段代码，从而生成 H-code）搜到特殊码 HS65001#-6C@1401B1F70，换算成"模块+RVA"就是 HS65001#-6C@1B1F70:Amakano3.exe（游戏基址 0x140000000）。Aurora 已按「文件名+字节数+CRC32」把这条码写进实测表，开翻译自动带出；真机连翻 3 页 —— 義大「そうだな」、詩夢を抱きしめる腕に力を込める。、義大「これでちょっとは落ち着ける？」全部完整、译文 3/3、审计漏掉 0。文本是 UTF-8，所以用 S + 65001#（不是 WillPlus 那种 UTF-16 的 Q）。
<br>实测（2026-09-19）：x64 CLI 挂钩成功（`管道已连接`、一排 GDI 钩子全部注入），进文本画面连翻 3 页**没有任何线程吐字**。原因查清了 —— 引擎把台词交给 `emotedriver.dll`（33 MB）**自己渲染**，该 DLL 只导入 `KERNEL32` + `D3DCOMPILER_47`，走 D3D11 + 自绘字形，**根本不调用 GDI 文本 API**，所以 `TextOutW`/`ExtTextOutW`/`GetGlyphOutlineW` 全是空跑。Textractor x64 里也没有 Artemis/Emote 引擎钩子（在 `TextractorCLI.exe`/`texthook.dll` 里查过，一个都没有）；LunaHook 有 `engine64/Artemis.cpp`，所以这一类引擎目前只有两条路：**LunaTranslator 的专用钩子**，或者 **OCR**。<br>**另见**：[Textractor 分支（Chenx221）的 Artemis x64 实现](textractor-fork-artemis.md) —— 它用「合成 pushad 帧」+ 特征码扫描，把文本指针从 **RDX/RDI 寄存器**里读出来（UTF-8），并提示 Artemis 代码区比老引擎大（3 MB）。这给我们的提示是：查找器要连**寄存器里的指针**一起采样。
**参考**：博客给出的 Artemis 解决方案是「固定地址的用户钩子」（实测另一款游戏是 HS65001#-44@15491A:<exe名>，即 UTF-8 字符串 + exe 内偏移）；SExtractor 则是**文件级**脚本提取（解包后正则抽取/回填，用来做汉化补丁，不是实时）。两者都没有可直接套到别的 Artemis 作品上的通用地址。
<br>Aurora 内置 OCR 可直接用：实测能读出 `詩夢 / 「よりによって、どうしてわたしが乗った時に……」` 这类台词；默认框选区会把名字框和边框框进来（读成 `こ - んなの`、`- ー - 「…」`），已在 `tidy_ocr_text` 里清掉（连字符 / 孤立长音符 / 标点两侧空格），在「翻译」面板里框紧对话框效果更好 |
...
## 五、自研「钩子查找器」（实验性，2026-09-19 加入）

翻译开了却一条文本都抓不到时，「翻译」面板里有「找不到文本？开始侦测」按钮：Aurora 自己
OCR 取当前台词 → 在内存里定位它的缓冲区（精确匹配；查不到就**差分法**：内存哈希快照 +
自动翻页 + 找变化区域里新出现的台词）→ 以调试器身份附加、给所有线程布**硬件数据断点** →
抓出「访问这条文本的指令 + 它落在哪个栈槽」→ 生成 Textractor H-code → 自动翻页验证 → 存成
该游戏的专用码。**不注入 DLL、不批量挂钩**（所以没有 Misaka 那种满屏闪烁），只在你手动点
按钮后才跑，随时可中止。

- 已验证：H-code 生成规则（拿 アマカノ３ 的 HS65001#-6C@1401B1F70 与少女之剑的
  HQ-4@A22E:AdvHD_crack.exe 两条真机码校准，含「负偏移要再 +4」这个坑）；
  调试器链路（合成目标进程里能稳定抓到访问指令、栈槽偏移与预期一致、结束后正确分离）。
- **已跑通（2026-09-19 真机）**：改成 Misaka 那种「采样收集候选」后，アマカノ３ 上自动找到并保存了 HS65001#20@38A78:emotedriver.dll（UTF-8 + 引擎自绘 DLL 内部地址），游戏随即正常吐文本。
  所以这个功能目前定位是**实验性**：失败会给出明确提示并保持原状（不影响既有功能）。
  アマカノ３ 请继续用已固化的 HS65001#-6C@1B1F70:Amakano3.exe（自动带出）。
