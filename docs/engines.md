...
<br>实测（2026-09-19）：x64 CLI 挂钩成功（`管道已连接`、一排 GDI 钩子全部注入），进文本画面连翻 3 页**没有任何线程吐字**。原因查清了 —— 引擎把台词交给 `emotedriver.dll`（33 MB）**自己渲染**，该 DLL 只导入 `KERNEL32` + `D3DCOMPILER_47`，走 D3D11 + 自绘字形，**根本不调用 GDI 文本 API**，所以 `TextOutW`/`ExtTextOutW`/`GetGlyphOutlineW` 全是空跑。Textractor x64 里也没有 Artemis/Emote 引擎钩子（在 `TextractorCLI.exe`/`texthook.dll` 里查过，一个都没有）；LunaHook 有 `engine64/Artemis.cpp`，所以这一类引擎目前只有两条路：**LunaTranslator 的专用钩子**，或者 **OCR**。<br>**参考**：博客给出的 Artemis 解决方案是「固定地址的用户钩子」（实测另一款游戏是 HS65001#-44@15491A:<exe名>，即 UTF-8 字符串 + exe 内偏移）；SExtractor 则是**文件级**脚本提取（解包后正则抽取/回填，用来做汉化补丁，不是实时）。两者都没有可直接套到别的 Artemis 作品上的通用地址。
