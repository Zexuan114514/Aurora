...
| **WillPlus / AdvHD** | 少女之剑与秘密的协奏曲 | ✅ 可用（**专用用户钩子码**，已固化） | Textractor 自带的 WillPlus 系钩子对不上这类 exe：`WillPlus` 报 `function call not found`、`WillPlusW/A` 找不到特征码、`WillPlus2` 把地址算到了 Intel 显卡驱动 `igc32.dll` 上（回显 `HQ-8*0@E8EB90:igc32.dll`，吐乱码）；只剩按字形抓的 `GetGlyphOutlineW`，而引擎字形有缓存 → 一行只抓到零星几个字（实测「出会って…打ち明けた」只抓到「出会　打明」）。**出路是自己给地址**：文本函数在 `AdvHD_crack.exe` 的 `0xA22E`（图像基址 0x400000，未开 ASLR），用 UTF-16 模式读它的参数 —— hook 码 `HQ-4@A22E:AdvHD_crack.exe`（`Q`=UTF-16 字符串、`S`=字节串、`V`=UTF-8，语法见 Textractor `host/hookcode.cpp`）。Aurora 已把这条实测记录写进指纹表（文件名+字节数+CRC32 三者匹配才启用），开翻译时自动带上；实测 5 句连续翻页全部完整、GDI 缺字版全部并掉（merged=9）、审计漏掉 0。**注意**：`attach` 之后不能立刻发钩子码（会把 CLI 顶掉、一行文本都收不到），要等 CLI 打印「管道已连接」再发 —— Aurora 已处理 |
