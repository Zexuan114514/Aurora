...
| **Emote / Artemis（x64，引擎自绘文字）** | アマカノ３（甜蜜女友 3） | ⚠️ **钩子不可行** → 用 OCR | 实测（2026-09-19）：x64 CLI 挂钩成功（`管道已连接`、一排 GDI 钩子全部注入），进文本画面连翻 3 页**没有任何线程吐字**。原因查清了 —— 引擎把台词交给 `emotedriver.dll`（33 MB）**自己渲染**，该 DLL 只导入 `KERNEL32` + `D3DCOMPILER_47`，走 D3D11 + 自绘字形，**根本不调用 GDI 文本 API**，所以 `TextOutW`/`ExtTextOutW`/`GetGlyphOutlineW` 全是空跑。Textractor x64 里也没有 Artemis/Emote 引擎钩子（在 `TextractorCLI.exe`/`texthook.dll` 里查过，一个都没有）；LunaHook 有 `engine64/Artemis.cpp`，所以这一类引擎目前只有两条路：**LunaTranslator 的专用钩子**，或者 **OCR**。<br>Aurora 内置 OCR 可直接用：实测能读出 `詩夢 / 「よりによって、どうしてわたしが乗った時に……」` 这类台词；默认框选区会把名字框和边框框进来（读成 `こ - んなの`、`- ー - 「…」`），已在 `tidy_ocr_text` 里清掉（连字符 / 孤立长音符 / 标点两侧空格），在「翻译」面板里框紧对话框效果更好 |
