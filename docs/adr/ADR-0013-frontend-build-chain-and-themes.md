# ADR-0013：引入前端构建链（Vue + Element Plus）与四套主题
...
1. 界面要**四套可切换的视觉风格 × 深/浅两态**。纯手写的 ES 模块没有组件层，
...
4. **布局只有一套，风格全在 CSS**：`src/styles/layout.css` 由 v1 的 `app.css` 机械迁移，
   ...
   四套主题（极光玻璃 / 展签式画廊 / 夜间放映厅 / 收藏架）× 深/浅两态只是令牌的不同取值。
...
- ✅ 组件能力与主题能力一步到位；四套风格共用一个 DOM，加风格 = 加一个 CSS 文件。
...
- ⚠️ 主题矩阵带来测试成本：截图基线按「深色 4 套 × 5 界面 + 浅色默认主题 5 张」分级，
  不做 8 套全量逐像素比对。已落在 `tools/visual.py` + `tools/baselines/theme-baseline.json`
