"""进程与会话原语（P3.9-d 从 gl/process.py 搬入）：启动/结束、进程树判定、会话时长统计。

进程树查询走 aurora.platform.proctree；会话结算规则来自 aurora.domain.session_rules。
"""
...
class ProcessManager:
    ...
    def start(self, game: dict, launcher: list[str] | None = None) -> dict:
        ...
        try:
            from aurora.platform import proctree
