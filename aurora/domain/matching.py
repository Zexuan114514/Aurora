"""匹配与打分（纯函数）：路径 → 关键词推断、名称归一化、候选相似度与打分。

P1 分层时从 `gl/detect.py` 搬出，并并入 `gl/sources/manager.py` 的打分判定；
行为由 `tests/fixtures/golden/matching.golden.json` 卡住。
"""
...
# --------------------------------------------------------------------------- #
# 以下来自 gl/sources/manager.py（P1 搬迁）：候选打分的纯判定，与源实现无关。
# --------------------------------------------------------------------------- #
