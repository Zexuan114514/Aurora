...
踩过的坑：很多日文 VN（含 WillPlus/AdvHD 的 少女之剑）**空格键是隐藏/显示
文本框**，不是推进 —— 用空格翻页看起来就像在「右键隐藏框」，词永远不动。
所以这里默认**左键点文本框区域**，其次回车；窗口不在前台一律不发（免得点到
别的程序上）。

用法：
    from vntext_advance import advance
    advance(hwnd)          # -> '左键点击 (x,y)' / '回车' / ''（没送出去）
"""
...
def ensure_dpi_awareness() -> str:
    """把自检脚本的进程设成 DPI 感知（启动器本身是 SYSTEM_AWARE）。

    不设的话：GetWindowRect 给「虚拟化」坐标、而屏幕 BitBlt 用物理坐标，
    两者对不上 —— 抓图会抓到隔壁窗口上（实测踩过这个坑）。
    """
    try:
...
def clickable(hwnd: int, point) -> bool:
    ...
    try:
        ...
        if not target:
            return False
        GA_ROOT = 2
        return int(user32.GetAncestor(target, GA_ROOT) or 0) == int(hwnd)
    except Exception:
        return False
